...
export type CharacterBrowserItem = {
  id: string;
  name: string;
...
export function CharacterBrowserSurface({
  characters,
}: {
      ...
      <section className="grid gap-3">
        {characters.length === 0 ? (
          <article className="rounded-lg border border-slate-200 bg-white p-4">
            <p className="text-sm text-slate-600">No Characters synced from Notion yet.</p>
          </article>
        ) : (
          characters.map((character) => (
            ...
            >
              <Link
                ...
                href={`/entity-workspace/characters/${character.id}`}
              >
                {character.name}
              </Link>
