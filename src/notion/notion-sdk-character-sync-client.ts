...
import type { NotionCharacterPage, NotionCharacterSyncClient } from "./character-sync";

export function createNotionSdkCharacterSyncClient(
  client: Client,
): NotionCharacterSyncClient {
  return {
    async listCharacterPages(input) {
      const database = await client.databases.retrieve({
        database_id: input.databaseId,
      });
      const dataSourceId = getFirstDataSourceId(database);
      const pages: NotionCharacterPage[] = [];
      ...
      do {
        ...
        for (const result of response.results) {
          pages.push(toNotionCharacterPage(result));
        }
...
function toNotionCharacterPage(response: unknown): NotionCharacterPage {
  if (
    !isRecord(response) ||
    ...
    throw new Error("Notion returned a Character query result without page properties.");
  }
