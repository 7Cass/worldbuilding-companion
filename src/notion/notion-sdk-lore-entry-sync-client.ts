...
export function createNotionSdkLoreEntrySyncClient(
  client: Client,
): NotionLoreEntrySyncClient {
  return {
    async listLoreEntryPages(input) {
      const database = await client.databases.retrieve({
        database_id: input.databaseId,
      });
      const dataSourceId = getFirstDataSourceId(database);
      const pages: NotionLoreEntryPage[] = [];
      ...
      do {
        ...
        for (const result of response.results) {
          pages.push(toNotionLoreEntryPage(result));
        }
...
function toNotionLoreEntryPage(response: unknown): NotionLoreEntryPage {
  if (
    !isRecord(response) ||
    ...
    throw new Error("Notion returned a Lore Entry query result without page properties.");
  }
