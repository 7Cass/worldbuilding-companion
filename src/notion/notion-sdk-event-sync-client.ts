...
import type { NotionEventPage, NotionEventSyncClient } from "./event-sync";

export function createNotionSdkEventSyncClient(client: Client): NotionEventSyncClient {
  return {
    async listEventPages(input) {
      ...
      do {
        ...
        for (const result of response.results) {
          pages.push(toNotionEventPage(result));
        }
...
function toNotionEventPage(response: unknown): NotionEventPage {
  if (
    !isRecord(response) ||
    ...
    throw new Error("Notion returned an Event query result without page properties.");
  }
